model BatteryDischargeChargeEqualize "Discharge and charge idealized battery"
  ...
  CellStackTC battery1TC(CycleLife = 100, Np = 1, Ns = 1, RUL(fixed = true, start = .8), SOC(fixed = true, start = 1), cellData = cellData1, totalCharge(fixed = false)) annotation(
  ...
  CellStackTC battery2TC(CycleLife = 100, Np = 1, Ns = 1, RUL(fixed = true, start = .6), SOC(fixed = true, start = 1), cellData = cellData, totalCharge(fixed = false)) annotation(
    Placement(visible = true, transformation(origin = {44, -56}, extent = {{10, -10}, {-10, 10}}, rotation = 90)));
  CustomPulse customPulse(nRepeats = 60, pulseWidth = 120, waitTime = 60) annotation(
    Placement(visible = true, transformation(origin = {-88, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  dividingmodelequalize dividingmodelequalize1 annotation(
    Placement(visible = true, transformation(origin = {-32, 2}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  battery1TC.RUL = dividingmodelequalize1.RUL1;
  battery2TC.RUL = dividingmodelequalize1.RUL2;
  battery1TC.SOC = dividingmodelequalize1.SOC1;
  battery2TC.SOC = dividingmodelequalize1.SOC2;
  ...
  connect(gain.y, dividingmodelequalize1.Load) annotation(
    Line(points = {{-44, 0}, {-44, -14}, {-34, -14}, {-34, -6}}, color = {0, 0, 127}));
  connect(dividingmodelequalize1.Bat2Load, signalCurrent2.i) annotation(
    Line(points = {{-20, -2}, {-12, -2}, {-12, -60}}, color = {0, 0, 127}));
  connect(dividingmodelequalize1.Bat1Load, signalCurrent1.i) annotation(
    Line(points = {{-20, 6}, {-12, 6}, {-12, 40}}, color = {0, 0, 127}));
  annotation(
...
but then due to the losses more energy is consumed to recharge the battery.
    ...
    Diagram);
end BatteryDischargeChargeEqualize;

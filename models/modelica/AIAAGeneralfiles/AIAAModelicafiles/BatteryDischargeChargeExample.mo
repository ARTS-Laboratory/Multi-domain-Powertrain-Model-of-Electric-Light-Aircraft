model BatteryDischargeChargeExample "Discharge and charge idealized battery"
  ...
  CellStackTC battery2TC(CycleLife = 100, Np = 1, Ns = 1, RUL(fixed = true, start = .2), SOC(fixed = true, start = 1), cellData = cellData) annotation(
  ...
  testdividingmodel testdividingmodel1 annotation(
    Placement(visible = true, transformation(origin = {-24, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  battery1TC.RUL = testdividingmodel1.RUL1;
  battery2TC.RUL = testdividingmodel1.RUL2;
  battery1TC.SOC = testdividingmodel1.SOC1;
  battery2TC.SOC = testdividingmodel1.SOC2;
  ...
  connect(gain.y, testdividingmodel1.Load) annotation(
    Line(points = {{-44, 0}, {-44, -8}, {-26, -8}}, color = {0, 0, 127}));
  connect(testdividingmodel1.Bat1Load, signalCurrent1.i) annotation(
    Line(points = {{-12, 4}, {-12, 40}}, color = {0, 0, 127}));
  connect(testdividingmodel1.Bat2Load, signalCurrent2.i) annotation(
    Line(points = {{-12, -4}, {-12, -60}}, color = {0, 0, 127}));
  annotation(
...
but then due to the losses more energy is consumed to recharge the battery.
    ...
    Diagram);
end BatteryDischargeChargeExample;

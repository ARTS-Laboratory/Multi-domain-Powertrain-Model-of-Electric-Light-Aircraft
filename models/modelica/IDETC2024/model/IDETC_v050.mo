model IDETC_v050
  ...
  CellStackTC_adapted stack2(RUL(fixed = true, start = 0.4), SOCini = 1, cellParameters = cellParams, np = 12, ns = 96)  annotation(
  ...
  Modelica.Blocks.Sources.Trapezoid trapezoid(amplitude = 2300*(3.14159/60), falling = 20, nperiod = -1, period = 6600, rising = 18.4, width = 1720)  annotation(
    Placement(visible = true, transformation(origin = {222, 386}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  ...
  PowerSystems.AC3ph.Inverters.Select select(alpha0 = 2.094395102393195, use_vPhasor_in = false)  annotation(
    Placement(visible = true, transformation(origin = {12, 378}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  PowerSystems.Blocks.Signals.TransientPhasor vCtrl(ph_end = 0.5235987755982988, t_change = 0.1, t_duration = 0.1)  annotation(
    Placement(visible = true, transformation(origin = {-16, 406}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  inner PowerSystems.System system(dynType = PowerSystems.Types.Dynamics.FixedInitial, f_nom = 60)  annotation(
  ...
  PowerSystems.Mechanics.Rotational.Speed speed(tcst = 1, use_w_in = true)  annotation(
    Placement(visible = true, transformation(origin = {136, 344}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  Modelica.Blocks.Logical.TerminateSimulation terminateSimulation(condition = stack2.RUL <= 0)  annotation(
    Placement(visible = true, transformation(origin = {72, 418}, extent = {{-40, -4}, {40, 4}}, rotation = 0)));
  Modelica.Blocks.Sources.Trapezoid trapezoid1(amplitude = 200*(3.14159/60), falling = 0, nperiod = -1, period = 6600, rising = 1.6, startTime = 18.4, width = 90)  annotation(
    Placement(visible = true, transformation(origin = {226, 318}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  Modelica.Blocks.Math.Add add annotation(
    Placement(visible = true, transformation(origin = {174, 344}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  Real totpower;
equation
  totpower = abs(der (DCmeter.i)/3600);
  stack1.staticResistance.cellCalculator.SOC = loadsplitter.SOC1;
  stack2.staticResistance.cellCalculator.SOC = loadsplitter.SOC2;
  loadsplitter.Load = if trapezoid.y > 0 then DCmeter.i else -32;
  stack1.RUL = loadsplitter.RUL1;
  stack2.RUL = loadsplitter.RUL2;
  load1.i = if trapezoid.y > 0 then 0.5*DCmeter.i else -16;
  load2.i = load1.i;
  //load1.i = if trapezoid.y > 0 then loadsplitter.Bat1Load else (if loadsplitter.SOC1 >= 1 then 0 else -16) ;
  //load2.i = if trapezoid.y > 0 then loadsplitter.Bat2Load else (if loadsplitter.SOC2 >= 1 then 0 else -16) ;
  ...
  connect(vCtrl.y, select.vPhasor_in) annotation(
    Line(points = {{-6, 406}, {20, 406}, {20, 388}, {18, 388}}, color = {0, 0, 127}, thickness = 0.5));
  ...
  connect(speed.flange, rotor.flange_b) annotation(
    Line(points = {{126, 344}, {112, 344}}));
  connect(add.y, speed.w_in) annotation(
    Line(points = {{164, 344}, {146, 344}}, color = {0, 0, 127}));
  connect(trapezoid.y, add.u1) annotation(
    Line(points = {{211, 386}, {200, 386}, {200, 350}, {186, 350}}, color = {0, 0, 127}));
  connect(trapezoid1.y, add.u2) annotation(
    Line(points = {{215, 318}, {200, 318}, {200, 338}, {186, 338}}, color = {0, 0, 127}));
  annotation(
